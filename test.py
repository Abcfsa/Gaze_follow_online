...
def get_model(_model):
    model,device,imgsz=detect.load_model(_model)
...
def main():
    ...
    if file==None:
        ...
        if sl.session_state.clicked==False:
            ...
            model, device, imgsz = get_model(head_model)
            # placeholder=sl.empty()
            df=pd.DataFrame(data=None,columns=['frame','xmin','ymin','xmax','ymax','id'])
            d_prog=sl.progress(0)
            for i in range(frame_num):
                img0=sl.session_state.frame_list[i]
                head_detect(_tracker=tracker,df=df,f_num=i+1,img0=img0,_model=model,
